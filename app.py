...
class AIScraper:
    ...
    async def analyze_with_gpt(self, content: str, instructions: str, keywords: List[str]) -> Dict:
        """Analyze content using GPT"""
        try:
            response = await openai.ChatCompletion.acreate(
                model="gpt-3.5-turbo",
                messages=[
                    {"role": "system", "content": "You are a web content analyzer. Analyze the provided content according to the instructions and provide matching keywords with associated 3D animations."},
    ...
    async def process_url(self, url: str, instructions: str, keywords: List[str] = []) -> Dict:
        ...
        try:
            ...
            for keyword in keywords:
                ...
                if keyword_lower in html.lower():
                    ...
                    video_url = f"/videos/{KEYWORD_VIDEO_MAP.get(keyword_lower, 'default_video.mp4')}"
...
async def scrape_url(request: ScrapeRequest):
    ...
    """
    try:
        keywords = request.keywords if request.keywords else list(KEYWORD_VIDEO_MAP.keys())
...
async def list_videos():
    ...
    for filename in os.listdir(video_dir):
        if filename.lower().endswith(('.mp4', '.webm', '.ogg')):  # Support multiple video formats
...
async def get_video(filename: str):
    video_path = os.path.join(os.path.dirname(__file__), "videos", filename)
